/*
Name: Ethan Chen
File Name: Stack ADT Implementing a Linked List (This is just the Stack file from the last project)
...
public class LLStack<Item> implements Iterable<Item>{ //LIFO (Last-In First-Out) Data Structure

    Node top;

    public LLStack() { // instantiates an empty stack

    }

    public LLStack(Node initialNode) { // instantiates a stack with a starting node
        top = initialNode;
    }

    public LLStack(int item) { // instantiates a stack given a data value for the starting node
        top = new Node(item);
    }
    ...
    //Iterable Implementation (Same as Queue)
    ...
    public static void main(String[] args) { // Interactive Console User Interface to test Stack methods
        LLStack firstStack = new LLStack();
